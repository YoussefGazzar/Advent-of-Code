require 'byebug'
if ARGV.empty?
	data =  DATA.readlines(chomp: true)
else
	data = File.readlines(ARGV.first, chomp: true)
end

result = data.chunk_while {!_1.empty?}.to_a.map {_1.reject(&:empty?)}
seeds, *rest = result
seeds = seeds.flat_map {_1.scan(/\d+/)}.map(&:to_i)
pp seeds
mappings = rest.each_with_object({}) do |line, mapping|
	key, *values = line 
	key = key.split.first
	values = values.each_with_object({}) do |value, hash|
		distnation, source, range = value.split.map(&:to_i)
		inner_key = (source...source+range)
		hash[inner_key] = distnation
	end
	mapping[key] = values
end

seeds = seeds.map do |seed|
	temp = seed
	# pp seed
	mappings.each do |_, entry|
		# pp entry
		entry.each do |k, v|
			# pp k
			# pp v
			if k.include?(temp) 
				temp = temp - k.first + v
				break
			end
		end
	end
	# pp temp
	temp
end

pp seeds
pp seeds.min
# pp mappings

__END__
seeds: 79 14 55 13

seed-to-soil map:
50 98 2
52 50 48

soil-to-fertilizer map:
0 15 37
37 52 2
39 0 15

fertilizer-to-water map:
49 53 8
0 11 42
42 0 7
57 7 4

water-to-light map:
88 18 7
18 25 70

light-to-temperature map:
45 77 23
81 45 19
68 64 13

temperature-to-humidity map:
0 69 1
1 0 69

humidity-to-location map:
60 56 37
56 93 4